...
public class AetherTinOreVeinFeature extends Feature<NoneFeatureConfiguration> {
    private static final Block GENERATED_BLOCK = ModBlocks.TIN_ORE_VEIN.get();

    public AetherTinOreVeinFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }

    @Override
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        int MAX_RADIUS = ModConfigs.server().worldGeneration.aetherTinOreVeinMaxRadius.get();

        if (ModConfigs.server().worldGeneration.generateAetherTinOreVein.get()) {
