...
public class EndLapisOreVeinFeature extends Feature<NoneFeatureConfiguration> {
    private static final Block GENERATED_BLOCK = ModBlocks.LAPIS_ORE_VEIN.get();

    public EndLapisOreVeinFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }

    @Override
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        int MAX_RADIUS = ModConfigs.server().worldGeneration.endLapisOreVeinMaxRadius.get();

        if (ModConfigs.server().worldGeneration.generateEndLapisOreVein.get()) {
