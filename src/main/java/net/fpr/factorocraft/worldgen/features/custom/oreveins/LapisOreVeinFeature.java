...
public class LapisOreVeinFeature extends Feature<NoneFeatureConfiguration> {
    private static final Block GENERATED_BLOCK = ModBlocks.LAPIS_ORE_VEIN.get();

    public LapisOreVeinFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }

    @Override
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        int MAX_RADIUS = ModConfigs.server().worldGeneration.lapisOreVeinMaxRadius.get();

        if (ModConfigs.server().worldGeneration.generateLapisOreVein.get()) {
