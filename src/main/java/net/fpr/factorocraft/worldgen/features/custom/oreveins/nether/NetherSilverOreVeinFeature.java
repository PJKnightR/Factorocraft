...
public class NetherSilverOreVeinFeature extends Feature<NoneFeatureConfiguration> {
    private static final Block GENERATED_BLOCK = ModBlocks.SILVER_ORE_VEIN.get();

    public NetherSilverOreVeinFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }

    @Override
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        int MAX_RADIUS = ModConfigs.server().worldGeneration.netherSilverOreVeinMaxRadius.get();

        if (ModConfigs.server().worldGeneration.generateNetherSilverOreVein.get()) {
