...
public class NetherOsmiumOreVeinFeature extends Feature<NoneFeatureConfiguration> {
    private static final Block GENERATED_BLOCK = ModBlocks.OSMIUM_ORE_VEIN.get();

    public NetherOsmiumOreVeinFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }

    @Override
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        int MAX_RADIUS = ModConfigs.server().worldGeneration.netherOsmiumOreVeinMaxRadius.get();

        if (ModConfigs.server().worldGeneration.generateNetherOsmiumOreVein.get()) {
