...
public class SilverOreVeinFeature extends Feature<NoneFeatureConfiguration> {
    private static final Block GENERATED_BLOCK = ModBlocks.SILVER_ORE_VEIN.get();

    public SilverOreVeinFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }

    @Override
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        int MAX_RADIUS = ModConfigs.server().worldGeneration.silverOreVeinMaxRadius.get();

        if (ModConfigs.server().worldGeneration.generateSilverOreVein.get()) {
