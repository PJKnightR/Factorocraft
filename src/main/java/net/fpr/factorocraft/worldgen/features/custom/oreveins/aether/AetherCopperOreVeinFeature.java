...
public class AetherCopperOreVeinFeature extends Feature<NoneFeatureConfiguration> {
    private static final Block GENERATED_BLOCK = ModBlocks.COPPER_ORE_VEIN.get();

    public AetherCopperOreVeinFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }

    @Override
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        int MAX_RADIUS = ModConfigs.server().worldGeneration.aetherCopperOreVeinMaxRadius.get();

        if (ModConfigs.server().worldGeneration.generateAetherCopperOreVein.get()) {
