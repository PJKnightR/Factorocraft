...
public class AetherSilverOreVeinFeature extends Feature<NoneFeatureConfiguration> {
    private static final Block GENERATED_BLOCK = ModBlocks.SILVER_ORE_VEIN.get();

    public AetherSilverOreVeinFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }

    @Override
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        int MAX_RADIUS = ModConfigs.server().worldGeneration.aetherSilverOreVeinMaxRadius.get();

        if (ModConfigs.server().worldGeneration.generateAetherSilverOreVein.get()) {
