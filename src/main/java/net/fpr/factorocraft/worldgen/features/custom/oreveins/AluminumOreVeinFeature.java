...
public class AluminumOreVeinFeature extends Feature<NoneFeatureConfiguration> {
    private static final Block GENERATED_BLOCK = ModBlocks.ALUMINUM_ORE_VEIN.get();

    public AluminumOreVeinFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }

    @Override
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        int MAX_RADIUS = ModConfigs.server().worldGeneration.aluminumOreVeinMaxRadius.get();

        if (ModConfigs.server().worldGeneration.generateAluminumOreVein.get()) {
