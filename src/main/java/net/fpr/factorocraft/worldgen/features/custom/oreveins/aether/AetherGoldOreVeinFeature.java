...
public class AetherGoldOreVeinFeature extends Feature<NoneFeatureConfiguration> {
    private static final Block GENERATED_BLOCK = ModBlocks.GOLD_ORE_VEIN.get();

    public AetherGoldOreVeinFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }

    @Override
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        int MAX_RADIUS = ModConfigs.server().worldGeneration.aetherGoldOreVeinMaxRadius.get();

        if (ModConfigs.server().worldGeneration.generateAetherGoldOreVein.get()) {
