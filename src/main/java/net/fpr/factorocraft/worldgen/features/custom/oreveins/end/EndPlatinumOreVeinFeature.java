...
public class EndPlatinumOreVeinFeature extends Feature<NoneFeatureConfiguration> {
    private static final Block GENERATED_BLOCK = ModBlocks.PLATINUM_ORE_VEIN.get();

    public EndPlatinumOreVeinFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }

    @Override
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        int MAX_RADIUS = ModConfigs.server().worldGeneration.endPlatinumOreVeinMaxRadius.get();

        if (ModConfigs.server().worldGeneration.generateEndPlatinumOreVein.get()) {
