...
public class NetherPlatinumOreVeinFeature extends Feature<NoneFeatureConfiguration> {
    private static final Block GENERATED_BLOCK = ModBlocks.PLATINUM_ORE_VEIN.get();

    public NetherPlatinumOreVeinFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }

    @Override
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        int MAX_RADIUS = ModConfigs.server().worldGeneration.netherPlatinumOreVeinMaxRadius.get();

        if (ModConfigs.server().worldGeneration.generateNetherPlatinumOreVein.get()) {
