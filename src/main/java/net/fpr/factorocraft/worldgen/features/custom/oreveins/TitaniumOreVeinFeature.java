...
public class TitaniumOreVeinFeature extends Feature<NoneFeatureConfiguration> {
    private static final Block GENERATED_BLOCK = ModBlocks.TITANIUM_ORE_VEIN.get();

    public TitaniumOreVeinFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }

    @Override
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        int MAX_RADIUS = ModConfigs.server().worldGeneration.titaniumOreVeinMaxRadius.get();

        if (ModConfigs.server().worldGeneration.generateTitaniumOreVein.get()) {
